...
//  Served by server.js from /public, with a real Postgres-backed
//  booking API at /api/bookings (see server.js and db.js).
// ═══════════════════════════════════════════════════

// ── IMAGE HELPER ──────────────────────────────────
// Images are served locally from the /Images folder (same server, same
// origin) rather than fetched from GitHub. The GitHub API is still used
// below purely as a convenience: it lists filenames in the repo's Images/
// folder so a destination/package can be fuzzy-matched to a photo without
// you needing to rename files to an exact expected name. If you'd rather
// not depend on GitHub's API at all, filenames can be set directly on each
// destination/package/gallery/testimonial entry instead.
...
// Submits to this site's own server (server.js → POST /api/bookings →
// Postgres, see db.js). Same-origin, so this works identically on
// localhost during development and once deployed.
var BOOKING_FORM_ENDPOINT = '/api/bookings';
...
var PACKAGES = [
  { id:'p1', name:'Maasai Mara Classic Safari', dest:'Kenya', duration:'3 Days', emoji:'🦁',
    ...
    itinerary:[
      { day:'Day 1', title:'Arrive in Maasai Mara', desc:'Drive in via the Great Rift Valley escarpment, arriving in time for lunch at camp. Afternoon sunset game drive in search of lion, elephant and cheetah, then dinner and overnight at camp.' },
      { day:'Day 2', title:'Full Day Game Drive', desc:'Full day exploring the reserve. Morning and afternoon drives seeking lions, leopards, and elephant herds.' },
      { day:'Day 3', title:'Breakfast → Nairobi', desc:'Breakfast at camp, then checkout and drive back to Nairobi, with an optional stop at a Maasai village along the way. Drop-off at hotel/airport.' },
    ],
  ...
  { id:'p3', name:'Kenya Grand Safari', dest:'Kenya', duration:'6 Days', emoji:'🌅',
    ...
    itinerary:[
      { day:'Day 1', title:'Nairobi → Tsavo West', desc:'Drive to Tsavo West National Park, arriving in time for lunch. Evening game drive through volcanic landscapes and lava fields in search of the park\'s red elephants.' },
      { day:'Day 2', title:'Tsavo West → Amboseli', desc:'Transfer to Amboseli, arriving for lunch. Evening game drive with classic elephant herds set against the Kilimanjaro backdrop.' },
      { day:'Day 3', title:'Amboseli → Lake Naivasha', desc:'Early morning game drive in Amboseli before transferring to Lake Naivasha, with an afternoon boat ride on the lake.' },
      { day:'Day 4', title:'Naivasha → Masai Mara', desc:'Transfer into the Mara ecosystem, arriving in time for an evening game drive across the reserve\'s predator-rich savanna.' },
      { day:'Day 5', title:'Masai Mara Full Day', desc:'A full day in the Mara — the jewel of Kenya. Balloon safari available (extra cost).' },
      { day:'Day 6', title:'Masai Mara → Nairobi', desc:'Final game drive before transferring back to Nairobi to conclude the Grand Safari.' },
    ],
  ...
  { id:'p10', name:'Classic Kenya Safari', dest:'Kenya', duration:'7 Days', emoji:'🐘',
    ...
    highlights:['Amboseli Elephants','Kilimanjaro Views','Masai Mara Big Cats','Optional Balloon Safari','Lake Naivasha Boat Ride','Lake Nakuru Flamingos'],
    itinerary:[
      ...
      { day:'Day 3', title:'Amboseli → Lake Naivasha', desc:'A shift to freshwater lake country — papyrus fringes, fish eagles and hippos. Afternoon boat ride on Lake Naivasha on arrival.' },
      { day:'Day 4', title:'Naivasha → Lake Nakuru', desc:'Transfer to Lake Nakuru, an alkaline lake ringed by woodland and escarpment. Evening game drive among flamingos, rhino and buffalo.' },
      { day:'Day 5', title:'Nakuru → Masai Mara', desc:'Transfer into the Mara ecosystem\'s riverine and kopje habitat. Evening game drive on arrival across the reserve\'s predator-rich savanna.' },
      { day:'Day 6', title:'Masai Mara Full Day', desc:'Open plains, riverine acacia corridors and rocky kopjes concentrate wildlife. Hunting cheetahs, territorial lion prides and leopards near outcrops. Optional sunrise balloon safari.' },
      { day:'Day 7', title:'Masai Mara → Nairobi', desc:'Final game drive before transferring back to Nairobi to conclude the safari.' },
    ],
  ...
  { id:'p12', name:'Amboseli & Tsavo Adventure', dest:'Kenya', duration:'6 Days', emoji:'🌋',
    ...
    itinerary:[
      { day:'Day 1', title:'Nairobi → Tsavo East', desc:'Transfer to Tsavo East National Park. Wide red-soil plains scattered with acacia and thorn scrub set the scene for an afternoon game drive in search of the park\'s iconic red-dust elephants.' },
      { day:'Day 2', title:'Tsavo East Full Day', desc:'A full day of wide-ranging drives across Tsavo East\'s open plains, tracking large ungulate herds, red-dust elephants and resident predators.' },
      { day:'Day 3', title:'Tsavo East → Tsavo West', desc:'Transfer into Tsavo West\'s volcanic ridges and mixed woodland. Afternoon visit to Mzima Springs, where hippos and crocodiles gather in clear spring-fed water.' },
      { day:'Day 4', title:'Tsavo West → Amboseli', desc:'Scenic drive past the Shetani lava flows before transferring on to Amboseli, arriving in time for an evening game drive beneath Kilimanjaro.' },
      { day:'Day 5', title:'Amboseli Full Day', desc:'Marsh-fed floodplains beneath Kilimanjaro showcase elephant herding behaviour, hippos, and active lion and cheetah at prime hours. Long drives for behaviour observation and photography.' },
      { day:'Day 6', title:'Amboseli → Nairobi', desc:'Final game drive before transferring back to Nairobi to conclude the Amboseli and Tsavo circuit.' },
    ],
  ...
  { id:'p15', name:'Rift Valley & Mara Safari', dest:'Kenya', duration:'7 Days', emoji:'🦩',
    ...
    itinerary:[
      { day:'Day 1', title:'Nairobi → Lake Naivasha (Hell\'s Gate)', desc:'Morning visit to Hell\'s Gate National Park for dramatic gorge scenery, followed by lunch, an afternoon boat ride on Lake Naivasha, and a nature walk at Crescent Island. Overnight at Naivasha.' },
      { day:'Day 2', title:'Naivasha → Lake Nakuru', desc:'Transfer to Lake Nakuru, an alkaline lake ringed by woodland and escarpment, arriving in time for an afternoon game drive.' },
      { day:'Day 3', title:'Lake Nakuru Full Day', desc:'In-depth exploration of the shoreline and surrounding woodland for seasonal flamingo concentrations, pelicans, and the park\'s resident rhino and buffalo populations.' },
      { day:'Day 4', title:'Nakuru → Masai Mara', desc:'Transfer into the Mara ecosystem\'s riverine and kopje habitat, with lunch en route. Evening game drive on arrival and overnight in the reserve.' },
      { day:'Day 5', title:'Masai Mara Full Day', desc:'Extended drives track the Mara\'s big cats and large herbivore herds, with floodplain specialists adding to the day\'s species list.' },
      { day:'Day 6', title:'Masai Mara Full Day', desc:'A further full day of multi-area Mara exploration, with focused predator tracking across riverine belts and open plains.' },
      { day:'Day 7', title:'Masai Mara → Nairobi', desc:'Transfer back to Nairobi to conclude the Rift Valley and Mara circuit.' },
    ],
  ...
  { id:'p16', name:'Southern Kenya Highlights', dest:'Kenya', duration:'7 Days', emoji:'🏖️',
    ...
    itinerary:[
      ...
      { day:'Day 4', title:'Tsavo West → Tsavo East', desc:'Transfer across the park boundary into Tsavo East, with an afternoon game drive through open red-soil plains.' },
      ...
      { day:'Day 7', title:'Departure from Mombasa', desc:'Optional marine activities or a return transfer to Nairobi to conclude the highland-to-coast journey. This base itinerary includes one coastal night — ask us about extending to 3 nights in Diani/Mombasa with added beach activities (snorkelling, dhow cruise, Wasini Island trip) for an additional cost.' },
    ],
  ...
  { id:'p17', name:'Kenya Big Five Trail', dest:'Kenya', duration:'10 Days', emoji:'🐆',
    ...
    highlights:['Ol Pejeta Rhino Sanctuary','Lake Nakuru','Extended Masai Mara','Lake Naivasha Boat Ride','Amboseli Elephants & Kilimanjaro'],
    itinerary:[
      ...
      { day:'Day 7', title:'Masai Mara → Lake Naivasha', desc:'Transfer out of the Mara toward Lake Naivasha, arriving in the afternoon with time for an optional boat ride on the lake.' },
      { day:'Day 8', title:'Lake Naivasha → Amboseli', desc:'Transfer onward to Amboseli, with afternoon arrival into elephant country beneath Kilimanjaro.' },
      { day:'Day 9', title:'Amboseli Full Day', desc:'A full day of Amboseli drives and photography, focused on elephant herding behaviour and the mountain backdrop.' },
      { day:'Day 10', title:'Amboseli → Nairobi', desc:'Transfer back to Nairobi, closing out the Big Five trail.' },
    ],
  ...
  { id:'p19', name:'Grand Kenya Discovery', dest:'Kenya', duration:'10 Days', emoji:'⛰️',
    ...
    highlights:['Aberdares Highland Forest','Ol Pejeta Rhinos','Mount Kenya Slopes','Samburu Specialists','Masai Mara & Naivasha Finale'],
    itinerary:[
      ...
      { day:'Day 3', title:'Aberdares → Ol Pejeta', desc:'Transfer to Ol Pejeta Conservancy. Rhino sanctuary visit, conservancy walks and wildlife hides on private land renowned for black and white rhino.' },
      { day:'Day 4', title:'Ol Pejeta → Mount Kenya', desc:'Transfer to Mount Kenya\'s lower slopes — montane forest and riverine channels offer trail birding and high-altitude scans for mountain-adapted species.' },
      { day:'Day 5', title:'Mount Kenya → Samburu', desc:'A transfer north with en-route stops and possible raptor migration watches, arriving into Samburu by evening.' },
      { day:'Day 6', title:'Samburu Full Day', desc:'Specialist drives to locate the region\'s northern endemics — Grevy\'s zebra, gerenuk, reticulated giraffe — and bird-rich riverine patches.' },
      { day:'Day 7', title:'Samburu → Lake Nakuru', desc:'Transfer south to Lake Nakuru, with lakeshore viewing and escarpment raptor sightings, resident rhino, buffalo and seasonal flamingo flocks on arrival.' },
      { day:'Day 8', title:'Lake Nakuru → Masai Mara', desc:'Transfer into the Mara ecosystem, arriving in time for an evening game drive across predator-rich savanna.' },
      { day:'Day 9', title:'Masai Mara Full Day', desc:'A full day of Mara exploration, tracking the reserve\'s big cats and large herbivore herds across riverine belts and open plains.' },
      { day:'Day 10', title:'Masai Mara → Lake Naivasha → Nairobi', desc:'Depart the Mara and pass by Lake Naivasha for a boat cruise and close-range wildlife viewing, before the final transfer back to Nairobi.' },
    ],
  ...
  { id:'p35', name:'Ultimate East Africa Discovery', dest:'Multi-country', duration:'13 Days', emoji:'🌟',
    price:4800, currency:'USD', perPerson:true, accom:'luxury',
    tag:'multi', badge:'Ultimate Discovery', image:'ultimate-east-africa-discovery.jpg',
    desc:'The most complete East Africa itinerary on offer — highland Aberdares, Samburu\'s north, a Rift Valley lakes break, the Masai Mara, Serengeti, Ngorongoro Crater and Tarangire, in one thirteen-day grand tour.',
    highlights:['Aberdares Highland Forest','Samburu Northern Specialists','Lake Nakuru Break','Extended Masai Mara','Serengeti & Ngorongoro','Tarangire Baobabs'],
    itinerary:[
      ...
      { day:'Day 5', title:'Samburu → Lake Nakuru', desc:'A restful break in the journey south — transfer to Lake Nakuru with an evening game drive among flamingos, rhino and buffalo on arrival.' },
      { day:'Day 6', title:'Nakuru → Masai Mara', desc:'Transfer onward to the Mara with an evening game drive on arrival, entering predator-rich savanna.' },
      { day:'Day 7-8', title:'Masai Mara Extended Drives', desc:'Two full days of predator tracking and migration observation where seasonal, with an optional balloon safari over the plains.' },
      { day:'Day 9', title:'Mara → Serengeti Cross-Border', desc:'A remote Serengeti arrival and late-afternoon drive across the border into Tanzania\'s vast plains.' },
      { day:'Day 10', title:'Serengeti Full Day', desc:'Kopje and plains exploration with sustained game-viewing across the Serengeti\'s classic predator country.' },
      { day:'Day 11', title:'Serengeti → Ngorongoro Highlands', desc:'An ascent to the highlands with a crater-rim viewpoint at sunset.' },
      { day:'Day 12', title:'Ngorongoro Crater Drive', desc:'A whole-day crater drive with picnic lunch, focused on rhino sightings and dense herbivore packs within the caldera.' },
      { day:'Day 13', title:'Tarangire Half-Day → Arusha Departure', desc:'A morning Tarangire drive among baobabs and elephant herds, then a final transfer to Arusha/Kilimanjaro for departure.' },
    ],
...
function submitBooking(){
  ...
  // A temporary local reference shown while the request is in flight. The
  // server generates the real, authoritative reference and stores it in
  // Postgres — we swap in that value from the response before showing the
  // success modal, so what the person sees always matches what's on file.
  ...
  .then(function(res){
    if (!res.ok) return res.json().then(function(data){ throw new Error((data && data.error) || ('Booking request failed with status ' + res.status)); });
    return res.json();
  })
  .then(function(data){
    resetButton();
    showSuccessModal(firstName, destText, data.reference || ref);
    resetBookingForm();
  })
  .catch(function(err){
    ...
    resetButton();
    showToast(err.message && /Too many booking requests/.test(err.message)
      ? err.message
      : 'We could not send your booking request — please try again, or contact us directly by phone/WhatsApp/email.', 'error');
  });
}
