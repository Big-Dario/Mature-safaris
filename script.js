...
// ── IMAGE HELPER ──────────────────────────────────
// Every place in this site that shows a picture (destinations, packages,
// gallery, hero, about, logo, testimonials) now looks for a real photo
// file inside the /images folder first. If that file does not exist
// (or hasn't been added yet), it automatically falls back to the emoji
// placeholder so the site never looks broken.
//
// HOW TO ADD YOUR OWN PHOTOS:
// 1. Put your image files inside the "images" folder next to this HTML file.
// 2. Name them EXACTLY as listed in images/README.txt (also summarised
//    in the comments above each data array below, e.g. "kenya.jpg").
// 3. Refresh the page — your photo will appear automatically.
// You do NOT need to touch this JavaScript to add photos.
function imgOrEmoji(src, emoji, extraClass){
...
// ── DATA ─────────────────────────────────────────
// image: put your own photo at images/<filename> — e.g. images/kenya.jpg
var DESTINATIONS = [
  { id:'kenya',    name:'Kenya',    emoji:'🦁', flag:'🇰🇪', tag:'Maasai Mara · Amboseli · Tsavo', count:'15+ packages', color:'#2C1810', image:'images/kenya.jpg' },
  { id:'tanzania', name:'Tanzania', emoji:'🐘', flag:'🇹🇿', tag:'Serengeti · Ngorongoro · Kilimanjaro', count:'12+ packages', color:'#1e3a1e', image:'images/tanzania.jpg' },
  { id:'uganda',   name:'Uganda',   emoji:'🦍', flag:'🇺🇬', tag:'Gorilla Trekking · Bwindi · Murchison', count:'8+ packages', color:'#3a2a0a', image:'images/uganda.jpg' },
  { id:'zanzibar', name:'Zanzibar', emoji:'🏖️', flag:'🇹🇿', tag:'Spice Tours · Beach Retreats · Snorkelling', count:'6+ packages', color:'#0a2a3a', image:'images/zanzibar.jpg' },
];

// image: put your own photo at images/<filename> — e.g. images/maasai-mara-classic.jpg
var PACKAGES = [
  { id:'p1', name:'Maasai Mara Classic Safari', dest:'Kenya', duration:'3 Days', emoji:'🦁',
    ...
    tag:'kenya', badge:'Best Seller', image:'images/maasai-mara-classic.jpg',
  ...
  { id:'p2', name:'Amboseli & Kilimanjaro Safari', dest:'Kenya', duration:'3 Days', emoji:'🐘',
    ...
    tag:'kenya', badge:'', image:'images/amboseli-kilimanjaro.jpg',
  ...
  { id:'p3', name:'Kenya Grand Safari', dest:'Kenya', duration:'5 Days', emoji:'🌅',
    ...
    tag:'kenya', badge:'Popular', image:'images/kenya-grand-safari.jpg',
  ...
  { id:'p4', name:'Tanzania Serengeti Explorer', dest:'Tanzania', duration:'5 Days', emoji:'🐃',
    ...
    tag:'tanzania', badge:'Featured', image:'images/serengeti-explorer.jpg',
  ...
  { id:'p5', name:'Uganda Gorilla Trek', dest:'Uganda', duration:'4 Days', emoji:'🦍',
    ...
    tag:'uganda', badge:'Rare Experience', image:'images/uganda-gorilla-trek.jpg',
  ...
  { id:'p6', name:'Zanzibar Spice & Beach Retreat', dest:'Zanzibar', duration:'5 Days', emoji:'🌊',
    ...
    tag:'zanzibar', badge:'Romantic', image:'images/zanzibar-beach-retreat.jpg',
  ...
  { id:'p7', name:'Kenya & Tanzania Combined', dest:'Multi-country', duration:'8 Days', emoji:'🌍',
    ...
    tag:'multi', badge:'Best Value', image:'images/kenya-tanzania-combined.jpg',
  ...
  { id:'p8', name:'Lake Nakuru & Naivasha', dest:'Kenya', duration:'2 Days', emoji:'🦩',
    ...
    tag:'kenya', badge:'Budget Friendly', image:'images/lake-nakuru-naivasha.jpg',
...
// image: put your own photo at images/<filename> — e.g. images/gallery-lions.jpg
var GALLERY_ITEMS = [
  { emoji:'🦁', label:'Lions of the Mara', color:'#3D1A00', image:'images/gallery-lions.jpg' },
  { emoji:'🐘', label:'Elephant Herds', color:'#1A2E1A', image:'images/gallery-elephants.jpg' },
  { emoji:'🦒', label:'Giraffes at Sunset', color:'#3D2E00', image:'images/gallery-giraffes.jpg' },
  { emoji:'🦓', label:'Zebra Migration', color:'#1A1A2E', image:'images/gallery-zebras.jpg' },
  { emoji:'🦍', label:'Mountain Gorillas', color:'#1A3D00', image:'images/gallery-gorillas.jpg' },
  { emoji:'🌅', label:'Savannah Sunrise', color:'#3D1A00', image:'images/gallery-sunrise.jpg' },
  { emoji:'🐆', label:'Leopard in Acacia', color:'#2E1A00', image:'images/gallery-leopard.jpg' },
  { emoji:'🏖️', label:'Zanzibar Coast', color:'#00203D', image:'images/gallery-zanzibar-coast.jpg' },
];

// image: put a guest photo at images/testimonial-1.jpg, testimonial-2.jpg, testimonial-3.jpg (optional — initials show if missing)
var TESTIMONIALS = [
  ...
  { stars:5, text:'We have done safaris with other operators, but Mother Nature Safaris is on a completely different level. The attention to detail, the lodge selection, the guide — every aspect was perfectly curated. We will absolutely be back.', name:'Melissa C.', from:'New York, USA', initials:'MC', image:'images/testimonial-2.jpg' },
  { stars:5, text:'Witnessed the Great Migration in the Maasai Mara — a scene that will stay with me forever. The team arranged everything seamlessly. Our driver-guide John was phenomenal, always finding the action before anyone else.', name:'Thomas & Ana B.', from:'São Paulo, Brazil', initials:'TB', image:'images/testimonial-3.jpg' },
];
...
function imgOrEmojiInitials(src, initials){
...
function submitBooking(){
  ...
  // Try the real backend first (server.js -> POST /api/bookings).
  // If the site is opened as a static file (no server running), or the
  // request fails for any reason, we fall back to a locally generated
  // reference number so the form still works end-to-end.
  fetch('/api/bookings', {
    method: 'POST',
  ...
  .catch(function(){
    // No server available (e.g. opened directly as a file, or offline) —
    // still confirm the request locally so the user isn't blocked.
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  // put your own logo at images/logo.png (small square works best)
  document.getElementById('navLogoIcon').innerHTML = imgOrEmoji('images/logo.png','🌿');
  document.getElementById('footerBrandTitle').innerHTML = imgOrEmoji('images/logo.png','🌿') + ' Mother Nature Safaris';
  // put a wide landscape photo at images/hero-background.jpg
  document.getElementById('heroBg').innerHTML = imgOrEmoji('images/hero-background.jpg','');
  // put a photo representing your company/team at images/about-photo.jpg
  document.getElementById('aboutImgFrame').innerHTML = imgOrEmoji('images/about-photo.jpg','🐘');
