...
var BOOKING_FORM_ENDPOINT = 'https://formspree.io/f/YOUR_FORM_ID'; // ← replace YOUR_FORM_ID
...
function loadImageFileList(){
  return fetch(IMAGE_FOLDER_API)
    ...
    .then(function(data){
      return data.filter(function(item){ return item.type === 'file'; })
...
function renderHeroSlideshow(){
  ...
  var baseFile = resolvedImage('hero-background.jpg');

  // Permanent base layer (hero-background.jpg) painted first, so it always
  // shows through — while slides are loading, in the crossfade gaps, and
  // as the fallback if every numbered slide fails to load.
  host.innerHTML =
    '<img src="' + img(baseFile) + '" alt="" loading="eager" onerror="this.remove()">' +
...
document.addEventListener('DOMContentLoaded', function(){
  // Ask GitHub what's actually in the Images/ folder right now, so uploaded
  // files can be auto-matched to destinations/packages by name — no exact
  // filename or code edits required. If this call is slow or fails (rate
  // limit, offline, etc.) we give up after 4s and fall back to the old
  // exact-filename behavior so the site still renders normally either way.
  var listingPromise = loadImageFileList();
  var timeoutFallback = new Promise(function(resolve){ setTimeout(function(){ resolve([]); }, 4000); });
  Promise.race([listingPromise, timeoutFallback]).then(function(fileList){
