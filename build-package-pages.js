...
//  safari package in PACKAGES (read out of public/script.js) into
//  /public/packages/.
//
//  WHY THIS EXISTS:
...
//
//  Re-run this any time PACKAGES changes in public/script.js, then commit
//  the regenerated files in /public/packages/ along with the updated
//  public/sitemap.xml.
// ═══════════════════════════════════════════════════════════
...
// Images are served locally from /public/Images by server.js (same origin
// as the page) — no external CDN dependency. Two different forms are
// needed:
//   - relImg(): a page-relative path, for actual <img src> tags on the
//     generated page (which lives one folder below /public, in
//     /public/packages/, hence the leading "../").
//   - absImg(): a fully-qualified absolute URL, required for og:image,
//     twitter:image, and JSON-LD "image" fields — social crawlers and
//     structured-data parsers don't resolve relative URLs against the page.
function relImg(filename) { return '../Images/' + filename; }
function absImg(filename) { return SITE_URL + '/Images/' + filename; }

// ── Load PACKAGES straight out of public/script.js so this can never
...
// Output now lives inside /public so server.js's express.static actually
// serves these pages (and so sitemap.xml, written below, is reachable at
// https://mothernaturesafaris.com/sitemap.xml rather than sitting unserved
// at the repo root).
const outDir = path.join(__dirname, 'public', 'packages');
...
PACKAGES.forEach(p => {
  ...
  const heroImgSrc = relImg(p.image);
  const socialImgUrl = absImg(p.image);
...
<nav class="navbar scrolled" style="position:relative">
  <div class="nav-inner">
    <a href="../index.html" style="display:flex;align-items:center;gap:10px;text-decoration:none">
      <div class="nav-logo-icon"><img src="${relImg('logo.png')}" alt="Mother Nature Safaris logo" onerror="this.replaceWith(Object.assign(document.createElement('span'),{className:'fallback-emoji',textContent:'🌿'}))"></div>
    ...
    <div style="display:grid;grid-template-columns:1.3fr 1fr;gap:48px;align-items:start">
      <div>
        <div style="border-radius:var(--r);overflow:hidden;aspect-ratio:16/10;background:linear-gradient(135deg,#2C1810,#3a2a0a);margin-bottom:28px">
          <img src="${heroImgSrc}" alt="${escapeHtml(p.name)}" style="width:100%;height:100%;object-fit:cover" loading="eager"/>
        </div>
  ...
  <div class="footer-inner">
    <div class="footer-grid">
      <div class="footer-brand">
        <h3><img src="${relImg('logo.png')}" alt="Mother Nature Safaris logo" style="width:28px;height:28px;object-fit:cover;border-radius:50%;vertical-align:middle;margin-right:8px" onerror="this.replaceWith(Object.assign(document.createElement('span'),{className:'fallback-emoji',textContent:'🌿'}))"> Mother Nature Safaris</h3>
...
// Written inside /public so it's actually served at /sitemap.xml by server.js
// (a sitemap sitting at the repo root, outside /public, would 404 in production).
fs.writeFileSync(path.join(__dirname, 'public', 'sitemap.xml'), sitemap);

console.log(`Generated ${PACKAGES.length} package pages in /public/packages/`);
console.log(`Updated public/sitemap.xml with ${sitemapEntries.length} URLs total`);
